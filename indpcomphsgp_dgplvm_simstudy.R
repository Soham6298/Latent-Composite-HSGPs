...
N <- 10  # higher N will take very long. Better to stick to <=30
dims <- c(2, 3)
# Set true input x
...
model_fit_deriv = foreach(i = 1:n_sim) %dopar% {
  ...
  for (j in 1:length(dims)) {
    ...
    out_gp_sigma_grad <- compare_summary(model = gp_fit, 
                                         ...
                                         model_name = 'derivgp',
                                         sim_id = i)
    out_gp1[[i]][[j]] <- rbind(out_gp_x, out_gp_rho, 
...
compare_table <- rbind(compare_deriv, compare_hsgp)
...
compare_table$model_id = paste0(compare_table$sim_id, '_', 
                                ...
                                compare_table$vary,
                                compare_table$corr)
saveRDS(compare_table, 'indcomphsgp_deriv.rds')