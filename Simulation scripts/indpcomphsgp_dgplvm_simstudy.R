...
# model fit list
model_fit_deriv = foreach(i = 1:n_sim) %dopar% {
  library(rstan)
  library(posterior)
  library(data.table)
  set.seed(i)
...
compare_deriv <- rbindlist(model_fit_deriv)
...
doParallel::registerDoParallel(cl)
# model fit list
model_fit_halfderiv = foreach(i = 1:n_sim) %dopar% {
  library(rstan)
  library(posterior)
  library(data.table)
  set.seed(i)
...
compare_halfderiv <- rbindlist(model_fit_halfderiv)
...
out_hsgp_deriv <- list()
out_hsgp_deriv1 <- list()
cores = 50
cl <- parallel::makeCluster(cores, type="PSOCK")
doParallel::registerDoParallel(cl)
# model fit list
model_fit_hsgp_deriv = foreach(i = 1:n_sim) %dopar% {
  library(rstan)
  library(posterior)
  library(data.table)
  set.seed(i)
  out_hsgp_deriv1[[i]] <- list()
  for (j in 1:length(dims)) {
    y1 <- simdata[[i]][[j]][1:N,1:dims[j]]
    y2 <- simdata[[i]][[j]][(N+1):(2*N),1:dims[j]]
    x_obs <- simdata[[i]][[j]]$x_obs[1:N]
    ...
    out_hsgp_sigma_g <- compare_summary(model = hsgp_fit,
                                        ...
                                        model_name = 'hsgp_deriv', 
                                        sim_id = i)
    out_hsgp_deriv1[[i]][[j]] <- rbind(out_hsgp_x, out_hsgp_rho_f, out_hsgp_alpha_f, out_hsgp_sigma_f,
                                 out_hsgp_rho_g, out_hsgp_alpha_g, out_hsgp_sigma_g)
  }
  out_hsgp_deriv[[i]] <- rbindlist(out_hsgp_deriv1[[i]])
}
compare_hsgp_deriv <- rbindlist(model_fit_hsgp_deriv)
compare_table <- rbind(compare_deriv, compare_halfderiv, compare_hsgp, compare_hsgp_deriv)
## comparison table edit
compare_table$data_id = paste0(compare_table$sim_id, '_', 
                                compare_table$d, '_',
                                compare_table$m)
saveRDS(compare_table, 'indcompgp_deriv_simout_se.rds')
