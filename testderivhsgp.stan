...
data {
	...
	int<lower=0, upper=1> is_hsgp; // 0: exact, 1: approx
...
parameters {
  // HSGP link parameter
	array[is_hsgp==1 ? D:0] vector[is_hsgp==1 ? M:0] beta;
	// Gaussian link parameters
	matrix[is_hsgp==0 ? N:0 , is_hsgp==0 ? D:0] eta;
...
transformed parameters{
  ...
  vector<lower=0>[D] sigma;
	vector[N] x;
	matrix[N, D] f;
	vector[is_hsgp==1 ? M:0] diagSPD;
	matrix[is_hsgp==1 ? M:0, is_hsgp==1 ? D:0] SPD_beta;
	matrix[is_hsgp==1? N:0, is_hsgp==1? M:0] PHI;
  ...
  if (covfn == 0) {
    ...
    for (k in 1:D) {
     matrix[N, N] K = se_deriv(x, alpha[k], rho[k], delta);
      f[, k] = K * eta[, k]; 
     }
    }
  }
