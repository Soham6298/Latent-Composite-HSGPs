functions {
    ...
    S = sqrt(2*pi()) * sq_alpha * rho * (w^2) * exp(-0.5*(sq_rho)*(w^2));
    return S;
	}
}
...
data {
	...
	int<lower=1> M_g;      // no. of basis functions	for f=g
...
transformed parameters {
  ...
  if (is_corr==0) {
	...
	for (j in 1:D) {
	  for (m in 1:M_f) { 
	      diagSPD_f[m] = sqrt(spd_se(alpha_f[j], rho[j], sqrt(lambda(L, m))));
	    }
	  for (m in 1:M_g) {
	      diagSPD_g[m] = sqrt(spd_se_deriv(alpha_g[j], rho[j], sqrt(lambda(L, m))));
	   } 
